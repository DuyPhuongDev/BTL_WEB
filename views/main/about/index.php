<?php 
   include_once('views/main/navbar.php');
?>


<div class="container about-section mt-5">
        <h1 class="text-center mb-5">Về Chúng Tôi</h1>
        <!-- Company Introduction -->
        <section class="mb-5">
            <h2 class="text-center">Giới Thiệu Công Ty</h2>
            <p class="text-center mt-4">
                Chào mừng đến với **FashionHub**, thương hiệu thời trang hàng đầu với sứ mệnh mang đến phong cách sống đẳng cấp, hiện đại. 
                Chúng tôi luôn tự hào với những sản phẩm chất lượng cao, mang đậm tính thẩm mỹ và đổi mới.
            </p>
        </section>

        <!-- Core Values -->
        <section class="mb-5">
            <h2 class="text-center">Giá Trị Cốt Lõi</h2>
            <div class="row mt-4">
                <div class="col-md-4 text-center">
                    <i class="bi bi-brush-fill"></i>
                    <h4 class="mt-3">Sáng Tạo</h4>
                    <p>Luôn đổi mới trong thiết kế để mang lại giá trị độc đáo cho khách hàng.</p>
                </div>
                <div class="col-md-4 text-center">
                    <i class="bi bi-people-fill"></i>
                    <h4 class="mt-3">Khách Hàng Là Trọng Tâm</h4>
                    <p>Đặt khách hàng làm ưu tiên số một trong mọi hoạt động kinh doanh.</p>
                </div>
                <div class="col-md-4 text-center">
                    <i class="bi bi-heart-fill"></i>
                    <h4 class="mt-3">Tận Tâm</h4>
                    <p>Cống hiến với tất cả tâm huyết để tạo nên sản phẩm hoàn hảo.</p>
                </div>
            </div>
        </section>

        <!-- Team Section -->
        <section>
            <h2 class="text-center">Đội Ngũ Của Chúng Tôi</h2>
            <div class="row mt-4">
                <!-- Team Member 1 -->
                <div class="col-md-4 text-center team-member">
                    <img src="https://via.placeholder.com/150" alt="Team Member 1">
                    <h5 class="mt-3">Nguyễn Văn A</h5>
                    <p>Nhà Sáng Lập</p>
                </div>
                <!-- Team Member 2 -->
                <div class="col-md-4 text-center team-member">
                    <img src="https://via.placeholder.com/150" alt="Team Member 2">
                    <h5 class="mt-3">Trần Thị B</h5>
                    <p>Giám Đốc Thiết Kế</p>
                </div>
                <!-- Team Member 3 -->
                <div class="col-md-4 text-center team-member">
                    <img src="https://via.placeholder.com/150" alt="Team Member 3">
                    <h5 class="mt-3">Lê Hồng C</h5>
                    <p>Trưởng Phòng Marketing</p>
                </div>
            </div>
        </section>
    </div>

<?php
   include_once('views/main/footer.php');
?>

